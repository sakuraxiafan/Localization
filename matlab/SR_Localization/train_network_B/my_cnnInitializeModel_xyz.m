...
lastLayer = net.vars(end).name;

ns = {'Z', 'X', 'Y'};
for i=1:length(ns)
    names = ['prediction' ns{i}];
    conv = dagnn.Conv('size', [1 1 1024 opts.outputSize(3)]);
    net.addLayer(names, conv, lastLayer, names, {[names '_f'], [names '_b']});
    parms = net.layers(end).block.initParams();
    net.params(end-1).value = parms{1}; net.params(end).value = parms{2};
    net.vars(net.getVarIndex(names)).precious = 1;
    
    switch ns{i}
        case 'Z'
            net.addLayer(['loss' ns{i}], dagnn.Loss('loss', 'logistic'), ...
                {names,['label' ns{i}]}, ['loss' ns{i}]) ;
            net.addLayer(['binerr' ns{i}], dagnn.Loss('loss', 'binaryerror'), ...
                {names,['label' ns{i}]}, ['top1err' ns{i}]) ;
        case {'X', 'Y'}
            net.addLayer(['loss' ns{i}], dagnn.Loss('loss', 'softmaxlog'), ...
                {names,['label' ns{i}]}, ['loss' ns{i}]) ;
            net.addLayer(['top1err' ns{i}], dagnn.Loss('loss', 'classerror'), ...
                {names,['label' ns{i}]}, ['top1err' ns{i}]) ;
    end
end

% var_sizes = net.getVarSizes({'data', opts.imageSize})';
