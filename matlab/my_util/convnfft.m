function C = convnfft(A, B, shape, dims, options)
...
%   C = CONVNFFT(..., SHAPE, DIMS, useGpu)
%       useGpu is boolean flag, see next
%
%   C = CONVNFFT(..., SHAPE, DIMS, OPTIONS)
%
%   OPTIONS is structure with following optional fields
%       - 'useGpu', boolean. If useGpu is TRUE Jacket/useGpu FFT engine will be used
%       By default useGpu is FALSE.
...
%       02-Sep-2009: useGpu/JACKET option
...
if nargin<5  && isunix
    options = struct('useGpu', true);
elseif nargin<5 || isempty(options)
    options = struct();
elseif ~isstruct(options) % useGpu options
    options = struct('useGpu', options);
end
...
% useGpu enable flag
useGpu = getoption(options, 'useGpu', false);
% Check if Jacket is installed
% useGpu = useGpu && ~isempty(which('ginfo'));
...
%     end
    ...
    for dim=dims
        ...
        l = lfftfun(m+n-1);
        % We need to swap dimensions because useGpu FFT works along the
        % first dimension
        ...
        if l~=1, A = fft(A,l); B = fft(B,l); end;
        subs{dim} = ifun(m,n);
    end
else % Matlab FFT
    % Do the FFT
